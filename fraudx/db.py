...
from fraudx.security import sanitize_sql_identifier
...
def save_prediction(tx_id: str, timestamp: str, fraud_score: float,
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?)",
        (tx_id, timestamp, fraud_score, prediction, risk_level, json.dumps(raw_input))
    )
    conn.commit()
    conn.close()
...
def save_feedback(tx_id: str, is_fraud: bool, analyst: str, comment: str = ""):
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?)",
        (tx_id, int(is_fraud), analyst[:100], comment[:500])
    )
    conn.commit()
    conn.close()
...
def save_model_metrics(model_name: str, model_path: str,
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?, ?, ?, ?)",
        (model_name, model_path, f1, recall, precision, auc_pr, json.dumps(params))
    )
    conn.commit()
    conn.close()
